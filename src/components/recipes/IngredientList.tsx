...
interface IngredientListProps {
    ...
    formik: any;
    newIngredient: any;
    setNewIngredient: any;
}

export default function IngredientList({action, image, modalVisible, setModalVisible, formik, newIngredient, setNewIngredient}: IngredientListProps) {
    ...
    const ingredientsTypes = useMemo(() => {
        return formik?.values?.recipe_ingredients?.length > 0
            ? formik?.values?.recipe_ingredients
                  ...
                  .filter(recipe => recipe.action === action)
            : [];
    }, [formik?.values?.recipe_ingredients]);

    console.log('formik?.values ingredientsTypes', formik?.values)
    console.log('ingredientsTypes', ingredientsTypes)
    ...
    const IngredientItem = React.memo(({ingredient, onEdit, onDelete}) => {
        return (
            <View style={styles.containerFlatListRow}>
                ...
                />
                <DSvgButton
                    svg={<TrashSvg stroke={'#F24E41'} width={20} height={20} />}
                        ...
                        <View style={styles.containerFlatList}>
                            <FlatList
                                ...
                                keyExtractor={(item, index) =>
                                    index.toString() + formik.values.id
                                }
                            />
                        </View>
                    ) : (
